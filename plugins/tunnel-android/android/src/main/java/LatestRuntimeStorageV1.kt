...
private const val QUICK_STATE_PREFERENCES = "nelomai-quick-tunnel-state"
private val PLAINTEXT_PREFERENCE_NAMES = listOf(
    QUICK_STATE_PREFERENCES,
...
internal interface NativeRuntimeStorageAccess {
    ...
    fun validateQuickStateDestination(name: String)
}
...
class LatestRuntimeStorageV1 : RuntimeNativeStorageV1 {
    ...
    internal fun acknowledge(storage: NativeRuntimeStorageAccess) {
        ...
        for (name in remaining) {
            ...
            if (record != null) {
                ...
                storage.validateQuickStateDestination(name)
            }
        }
    ...
    companion object {
        ...
        private val legacyNames = records.map { it.first } + PLAINTEXT_PREFERENCE_NAMES
    }
}

internal class AndroidNativeRuntimeStorageAccess(
    private val context: Context,
    private val receiptFactory: (String) -> EncryptedRecordBackend = { name ->
        AndroidSecureEnvelopeBackend(context,
            preferenceName = "native-migration-receipt-$name", recordName = "receipt-v1",
            keyAlias = "native-migration-receipt-$name")
    },
    ...
    override fun validateQuickStateDestination(name: String) {
        check(name in PLAINTEXT_PREFERENCE_NAMES)
        check(context.getSharedPreferences(AndroidRuntimeNamespace.record(name), Context.MODE_PRIVATE).all.isNotEmpty()) {
            "native_migration_destination_missing"
        }
    }
    override fun migrateQuickState() {
        // These plaintext projections are non-secret, but their retry markers
        // must move together instead of aliasing the legacy preferences.
        for (stateName in PLAINTEXT_PREFERENCE_NAMES) {
            ...
            if (stateReceipt.read() == null) {
                ...
                source.all.forEach { (key, value) -> when (value) {
                    ...
                    else -> error("unsupported_native_plaintext_state")
                } }
    ...
    override fun receipt(name: String) = receiptFactory(name)
