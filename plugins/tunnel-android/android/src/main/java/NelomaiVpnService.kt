...
private const val REDUNDANT_STOP_RETRY_MILLIS = 1_000L
...
/** Owns one accepted redundant start until readiness or cleanup acknowledges its terminal result. */
internal class RedundantStartOperationGate {
    ...
    private data class Pending(
        ...
        var lifecycleFinished: Boolean = false,
    ...
    private fun completeLocked(operationId: String, action: () -> Unit): Boolean {
        ...
        try {
            ...
            if (current.lifecycleFinished) pending = null
        }
        return true
    }
}
...
internal class RedundantRevokeLifecycleGate {
    ...
    private var frameworkCompleted = false

    fun begin(): Boolean = synchronized(gate) {
    ...
    fun hasPendingCleanup(): Boolean = synchronized(gate) { cleanupPending }

    fun needsFrameworkCompletion(): Boolean = synchronized(gate) {
        begun && !frameworkCompleted
    }

    fun cleanupFinished() = synchronized(gate) {
        cleanupPending = false
    }

    fun completeFramework(action: () -> Unit): Boolean {
        val shouldComplete = synchronized(gate) {
            if (frameworkCompleted) false else {
                frameworkCompleted = true
                true
            }
        }
        if (shouldComplete) action()
...
internal class RedundantVpnOwnerSlot {
    @Volatile
    private var installed: InstalledRedundantVpnOwner? = null

    fun snapshot(): InstalledRedundantVpnOwner? = installed
    ...
    @Synchronized
    fun removeIf(startOperationId: String): InstalledRedundantVpnOwner? {
        val current = installed?.takeIf { it.startOperationId == startOperationId }
            ?: return null
        installed = null
        return current
    }
...
/** Retains process owners by identity until their local native session closes successfully. */
internal class RetainedRedundantOwnerCleanup {
    private val gate = Any()
    private val retained = java.util.IdentityHashMap<RedundantVpnProcessOwner, Unit>()

    fun closeOrRetain(owner: RedundantVpnProcessOwner): Boolean {
        val shouldClose = synchronized(gate) {
            if (retained.containsKey(owner)) false else {
                retained[owner] = Unit
                true
            }
        }
        if (!shouldClose) return false
        val closed = runCatching(owner::closeLocal).getOrDefault(false)
        if (closed) synchronized(gate) { retained.remove(owner) }
        return closed
    }

    /** Retries each retained owner once and returns whether another retry remains necessary. */
    fun retry(): Boolean {
        val snapshot = synchronized(gate) { retained.keys.toList() }
        snapshot.forEach { owner ->
            if (runCatching(owner::closeLocal).getOrDefault(false)) {
                synchronized(gate) { retained.remove(owner) }
            }
        }
        return hasPending()
    }

    fun hasPending(): Boolean = synchronized(gate) { retained.isNotEmpty() }
}
...
private data class PendingRedundantStopAttempt(
...
internal fun dispatchRedundantWork(
...
/** Blocks new recovery-v2 starts while a stop request resolves durable ownership. */
internal class RedundantStopLookupBarrier {
    private val gate = Any()
    ...
    fun begin(): Long = synchronized(gate) {
        nextToken += 1
        nextToken.also(pending::add)
    }

    fun complete(token: Long): Boolean = synchronized(gate) { pending.remove(token) }

    fun hasPending(): Boolean = synchronized(gate) { pending.isNotEmpty() }
}

internal fun <T : Any> dispatchWithRedundantStopLookupBarrier(
    dispatcher: RedundantVpnWorkDispatcher,
    fallbackDispatcher: RedundantVpnWorkDispatcher,
    barrier: RedundantStopLookupBarrier,
    resolveInMemory: () -> T?,
    resolveDurable: () -> T,
    ...
    fun reject() {
        try {
            postToCaller { finish(onRejected) }
        } catch (_: Throwable) {
            finish(onRejected)
        }
    }
    dispatchRedundantWork(
        ...
        action = {
            val resolved = try {
                resolveInMemory() ?: resolveDurable()
            } catch (_: Throwable) {
                reject()
                return@dispatchRedundantWork
            }
            try {
                postToCaller { finish { complete(resolved) } }
            } catch (_: Throwable) {
                reject()
            }
        },
...
class NelomaiVpnService : GoBackend.VpnService() {
    ...
    private val redundantHealthTick = object : Runnable {
        override fun run() {
            if (redundantVpnOwner != null && !redundantStartBlocked()) {
                redundantWork.tick {
    ...
    private lateinit var redundantTotalLossLifecycle: RedundantTotalLossLifecycle
    ...
    override fun onCreate() {
        ...
        redundantCancelTombstones = RedundantCancelTombstoneStore(
            AndroidRedundantCancelTombstoneBackend(applicationContext),
        )
        when (val tombstone = redundantCancelTombstones.read()) {
            is RecoveryStoreResult.Success -> tombstone.value?.let {
                pendingRedundantStop = PendingRedundantStop(
                    ...
                    localClosed = true,
                    allowRestart = it.allowRestart,
                )
            }
            is RecoveryStoreResult.Failure -> {
                redundantCancelTombstoneUnreadable = true
                TunnelLog.warning("redundant.cancel_tombstone_read_failed", tombstone.code)
            }
        }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when {
            ...
            intent == null && backgroundLogoutState() != BackgroundLogoutReadState.NONE -> {
    ...
    private fun handleClientStart(intent: Intent) {
        ...
        backgroundLogoutClientStartFailure(backgroundLogoutState())?.let { code ->
        ...
        try {
            ...
            val started = coordinator.start(
                ...
                onPrimaryStarted = {
                    restoreHandler.post {
                        try {
                            when (redundantPrimaryReadyDisposition(
                                recoveryStore.read(),
                                startCancelled = redundantStartOperation.isCancelled(
                                    clientOperationId,
                                ),
                            )) {
                                ...
                                RedundantPrimaryReadyDisposition.CANCELLED -> {
                                    beginFailClosedRedundantStop(clientOperationId, coordinator)
                                }
    ...
    private fun createRedundantCoordinator(
        ...
        if (!installed) {
            throw BackgroundConnectionException("redundant_start_cancelled")
        }
        restoreHandler.post(redundantHealthTick)
        return coordinator to native
    }

    private fun handleRedundantTotalLoss(
        ...
        val disposition = redundantTotalLossCommandDisposition(
            ...
            installedOwnerMatches = installed?.owner === owner,
            stopPending = pendingRedundantStop != null,
    ...
    private fun dispatchRedundantResume(
        ...
        if (durableTransaction.retry.totalLossRestartReplay != null) {
            val replayServiceGeneration = serviceGeneration
            restoreHandler.post {
                if (serviceDestroyed ||
                    replayServiceGeneration != VPN_PROCESS_SERVICE_GENERATION.get() ||
                    pendingRedundantStop != null
                ) {
                    return@post
                }
                val current = (recoveryStore.read() as? RecoveryStoreResult.Success)
                    ?.value?.redundantTransaction?.takeIf {
                        it.startOperationId == durableTransaction.startOperationId &&
                            it.retry.totalLossRestartReplay != null
                    } ?: return@post
                beginFailClosedRedundantStop(
                    startOperationId = current.startOperationId,
                    owner = redundantOwnerForOperation(current.startOperationId),
                    allowRestart = true,
                )
            }
            return true
        ...
        redundantWork.resume {
            val current = activeRedundantTransactionForCurrentWork(
                recovery = recoveryStore.read(),
                expectedStartOperationId = transaction.startOperationId,
            ) ?: return@resume
    ...
    private fun finishRedundantStart(operationId: String) {
        if (pendingRedundantStop?.startOperationId != operationId) {
            redundantStartOperation.finish(operationId)
        }
    }
    ...
    private fun beginFailClosedRedundantStop(
        ...
        val pending = existing?.copy(
            ...
            localClosed = if (existing.owner == null && owner != null) {
                ...
                existing.localClosed
            },
            allowRestart = existing.allowRestart && allowRestart,
        ...
        val tombstone = when (val result = redundantCancelTombstones.persist(
            startOperationId,
            pending.allowRestart,
        )) {
        ...
        val effectiveTombstone = tombstone ?: pending.tombstone
        val durablePending = pending.copy(
            tombstone = effectiveTombstone,
            allowRestart = pending.allowRestart &&
                (effectiveTombstone?.allowRestart ?: true),
        )
        pendingRedundantStop = durablePending
        if (durablePending.tombstone != null) redundantCancelTombstoneUnreadable = false
        val durableStopFence = if (durablePending.allowRestart) {
            true
        } else {
            val stopOperationId = durablePending.tombstone?.stopOperationId
            ...
            when (val result = recoveryStore.cancelRedundantIntentAndDeferStop(
                ...
                is RecoveryStoreResult.Failure -> {
                    TunnelLog.warning("redundant.stop_fence_failed", result.code)
                    false
                }
            }
        }
    ...
    private fun retryPendingRedundantStop() {
                ...
                restoreHandler.post {
                    handleRedundantTombstoneRead(
                        ...
                        install = { tombstone ->
                            ...
                            pendingRedundantStop = PendingRedundantStop(
                                startOperationId = tombstone.startOperationId,
                                tombstone = tombstone,
                                owner = null,
                                serviceGeneration = serviceGeneration,
                                localClosed = true,
                                allowRestart = tombstone.allowRestart,
                            )
                            schedulePendingRedundantStopRetry()
                        },
                        retry = ::schedulePendingRedundantStopRetry,
                        resumeDurableWork = {
                            redundantCancelTombstoneUnreadable = false
        ...
        executeRedundantCleanup {
            val tombstone = pending.tombstone ?: when (
                ...
                is RecoveryStoreResult.Success -> persisted.value
                is RecoveryStoreResult.Failure -> {
                    restoreHandler.post { schedulePendingRedundantStopRetry() }
                    return@executeRedundantCleanup
                }
            }
            val recovery = recoveryStore.read()
            ...
            val result = when {
                ...
                transaction == null -> {
                    ...
                    val promotedForThisStop = envelope.intent.retry
                        .redundantTotalLossSourceStartOperationId == pending.startOperationId
                    if (!pending.allowRestart && promotedForThisStop) {
                        val cancelled = recoveryStore.cancelRedundantIntentAndDeferStop(
                            stopOperationId = tombstone.stopOperationId,
                            expectedStartOperationId = pending.startOperationId,
                        ) is RecoveryStoreResult.Success
                        RedundantRevokeResult(fenced = cancelled, stopped = cancelled)
                    } else {
                        val terminalForBarrier = if (pending.allowRestart) {
                            promotedForThisStop || !envelope.intent.desiredActive
                        } else {
                            !envelope.intent.desiredActive && envelope.leaseTransaction == null
                        }
                        RedundantRevokeResult(
                            fenced = terminalForBarrier,
                            stopped = terminalForBarrier,
                        )
                    }
                }
                transaction.startOperationId != pending.startOperationId -> {
                    ...
                    deferStop = {
                        val restartStillAllowed = pendingRedundantStop?.takeIf {
                            it.startOperationId == pending.startOperationId &&
                                it.serviceGeneration == pending.serviceGeneration
                        }?.allowRestart == true
                        val durableRestartAllowed =
                            (redundantCancelTombstones.read() as? RecoveryStoreResult.Success)
                                ?.value?.takeIf {
                                    it.startOperationId == tombstone.startOperationId &&
                                        it.stopOperationId == tombstone.stopOperationId
                                }?.allowRestart == true
                        val restartPrepared = restartStillAllowed &&
                            durableRestartAllowed &&
                            transaction.retry.totalLossRestartReplay != null
                        val fenced = if (restartPrepared) {
                            recoveryStore.deferRedundantStop(
                                stopOperationId = tombstone.stopOperationId,
                                expectedStartOperationId = pending.startOperationId,
                            )
                        } else {
                            recoveryStore.cancelRedundantIntentAndDeferStop(
                                stopOperationId = tombstone.stopOperationId,
                                expectedStartOperationId = pending.startOperationId,
                            )
                        }
                        fenced is RecoveryStoreResult.Success
                    },
            ...
            restoreHandler.post { completePendingRedundantStopAttempt(attempt) }
        }
    }

    private fun completePendingRedundantStopAttempt(attempt: PendingRedundantStopAttempt) {
        val current = pendingRedundantStop ?: return
        if (current.startOperationId != attempt.pending.startOperationId ||
            current.serviceGeneration != attempt.pending.serviceGeneration
        ) {
            return
        }
        pendingRedundantStop = current.copy(tombstone = attempt.tombstone)
        if (shouldCompleteRedundantCancellation(
                tombstonePersisted = true,
                localClosed = current.localClosed,
                cleanupStopped = attempt.result.stopped,
            )
        ) {
            redundantStartOperation.completeCancelled(current.startOperationId)
        }
        if (!attempt.result.fenced || !attempt.result.stopped) {
            completeRedundantStopWaiters(current.startOperationId, attempt.result)
            completeFrameworkRevokeAfterCleanupAttempt()
            schedulePendingRedundantStopRetry()
            return
        }
        val recoveryAfterCleanup = recoveryStore.read()
        val completionDisposition = redundantStopCompletionDisposition(
            ownerServiceGeneration = current.serviceGeneration,
            currentServiceGeneration = VPN_PROCESS_SERVICE_GENERATION.get(),
            recovery = recoveryAfterCleanup,
        )
        if (completionDisposition == RedundantStopCompletionDisposition.RETRY) {
            schedulePendingRedundantStopRetry()
            return
        }
        val cleared = redundantCancelTombstones.clear(attempt.tombstone)
        val durableAfterClear = if (cleared) {
            RecoveryStoreResult.Success(null)
        } else {
            redundantCancelTombstones.read()
        }
        when (redundantTombstoneClearDisposition(
            cleared = cleared,
            durable = durableAfterClear,
            ownerServiceGeneration = current.serviceGeneration,
            currentServiceGeneration = VPN_PROCESS_SERVICE_GENERATION.get(),
            expected = attempt.tombstone,
        )) {
            ...
            RedundantTombstoneClearDisposition.REPLAY_SUPERSEDING -> {
                val replacement = requireNotNull(
                    (durableAfterClear as RecoveryStoreResult.Success).value,
                )
                val replacementOwner = redundantOwnerForOperation(
                    replacement.startOperationId,
                )
                redundantStartOperation.finish(current.startOperationId)
                pendingRedundantStop = PendingRedundantStop(
                    startOperationId = replacement.startOperationId,
                    tombstone = replacement,
                    owner = replacementOwner,
                    serviceGeneration = serviceGeneration,
                    localClosed = replacementOwner == null,
                    allowRestart = replacement.allowRestart,
                )
                completeRedundantStopWaiters(current.startOperationId, attempt.result)
                completeFrameworkRevokeAfterCleanupAttempt()
                schedulePendingRedundantStopRetry()
            ...
            RedundantTombstoneClearDisposition.STALE -> {
                ...
                completeRedundantStopWaiters(current.startOperationId, attempt.result)
                completeFrameworkRevokeAfterCleanupAttempt()
                completeRedundantStopServiceLifecycle(current.serviceGeneration)
                return
            }
        ...
        pendingRedundantStop = null
        redundantStartOperation.finish(current.startOperationId)
        if (completionDisposition == RedundantStopCompletionDisposition.PUBLISH_STOPPED) {
            installRedundantVpnOwner(null)
            QuickTunnelController.updateState(
                applicationContext,
                SessionState.STOPPED,
    ...
    private fun completeRedundantStopServiceLifecycle(ownerServiceGeneration: Long) {
        if (serviceDestroyed) {
            redundantExecutor.shutdown()
            stopIfIdle()
        } else {
            redundantTotalLossLifecycle.onCleanupAcknowledged(ownerServiceGeneration)
        }
    }

    private fun detachCompletedRedundantOwner(completed: PendingRedundantStop) {
        val removed = redundantVpnOwnerSlot.removeIf(completed.startOperationId)
        if (removed == null) {
            completed.owner?.let(::closeOrRetainRedundantOwner)
            return
        }
        restoreHandler.removeCallbacks(redundantHealthTick)
        closeOrRetainRedundantOwner(removed.owner)
        redundantPhysicalNetworks?.stop()
        redundantPhysicalNetworks = null
    }

    private fun redundantOwnerForOperation(
        startOperationId: String,
    ): RedundantVpnProcessOwner? = redundantVpnOwnerSlot.snapshot()
        ?.takeIf { it.startOperationId == startOperationId }
        ?.owner

    private fun completeFrameworkRevokeAfterCleanupAttempt() {
        if (!redundantRevokeLifecycle.hasPendingCleanup()) return
        redundantRevokeLifecycle.cleanupFinished()
        redundantRevokeLifecycle.completeFramework(::invokeFrameworkVpnRevoke)
    }

    private fun completeRedundantStopWaiters(
        startOperationId: String,
        result: RedundantRevokeResult,
    ) {
    ...
    private fun redundantStartBlocked(): Boolean = redundantCleanupBlocksNewStarts(
    ...
    private fun activeRedundantTransactionForCurrentWork(
        recovery: RecoveryStoreResult<AndroidRecoveryEnvelope>,
        expectedStartOperationId: String?,
    ): AndroidRedundantTransaction? = activeRedundantTransactionForWork(
        recovery = recovery,
        expectedStartOperationId = expectedStartOperationId,
        pendingStop = pendingRedundantStop != null,
        tombstoneUnreadable = redundantCancelTombstoneUnreadable,
        stopLookupPending = redundantStopLookupBarrier.hasPending(),
        retainedOwnerCleanupPending = retainedRedundantOwnerCleanup.hasPending(),
    ...
    ): Boolean = shouldApplyConnectionIntentStep(
        pendingStop = pendingRedundantStop != null,
        tombstoneUnreadable = redundantCancelTombstoneUnreadable,
        envelope = envelope,
        stopLookupPending = redundantStopLookupBarrier.hasPending(),
    ...
    private fun scheduleRetainedRedundantOwnerRetry() {
        if (!retainedRedundantOwnerCleanup.hasPending()) return
        restoreHandler.removeCallbacks(retainedRedundantOwnerRetry)
        restoreHandler.postDelayed(
            retainedRedundantOwnerRetry,
            REDUNDANT_STOP_RETRY_MILLIS,
        )
    }

    private fun retryRetainedRedundantOwners() {
        dispatchRedundantWork(
            dispatcher = redundantWork,
            fallbackDispatcher = VPN_PROCESS_CLEANUP_WORK,
            action = {
                val pending = retainedRedundantOwnerCleanup.retry()
                restoreHandler.post {
                    if (pending) {
                        scheduleRetainedRedundantOwnerRetry()
            ...
            onRejected = ::scheduleRetainedRedundantOwnerRetry,
        )
    }

    private fun resolveRedundantStopFromMemoryOrRecovery(
        resolveInMemoryOperationId: () -> String?,
        ...
        dispatchWithRedundantStopLookupBarrier(
            ...
            barrier = redundantStopLookupBarrier,
            resolveInMemory = {
                resolveInMemoryOperationId()?.let {
                    ResolvedRedundantStopLookup(operationId = it, recovery = null)
                }
            },
            resolveDurable = {
                val recovery = recoveryStore.read()
                ResolvedRedundantStopLookup(
                    operationId = (recovery as? RecoveryStoreResult.Success)
                        ?.value
                        ?.let(resolveDurableOperationId),
                    recovery = recovery,
                )
            },
            ...
            onRejected = {
                complete(
                    ResolvedRedundantStopLookup(
                        operationId = null,
                        recovery = RecoveryStoreResult.Failure(
                            "redundant_recovery_dispatch_rejected",
                        ),
                    ),
                )
            },
    ...
    private fun handleCancelClientStart(intent: Intent) {
        ...
        val knownInMemory = pendingCancelled ||
            pendingRedundantStop?.startOperationId == clientOperationId ||
            redundantVpnOwnerSlot.snapshot()?.startOperationId == clientOperationId
        if (knownInMemory) {
            beginCancelledRedundantStartStop(clientOperationId)
            return
        }
        resolveRedundantStopFromMemoryOrRecovery(
            resolveInMemoryOperationId = {
                clientOperationId.takeIf {
                    redundantStartOperation.cancel(clientOperationId) ||
                        pendingRedundantStop?.startOperationId == clientOperationId ||
                        redundantVpnOwnerSlot.snapshot()?.startOperationId == clientOperationId
                }
            },
            resolveDurableOperationId = { envelope ->
                envelope.redundantTransaction?.startOperationId?.takeIf {
                    it == clientOperationId
                }
            },
        ) { lookup ->
            if (lookup.operationId == clientOperationId) {
                beginCancelledRedundantStartStop(clientOperationId)
                return@resolveRedundantStopFromMemoryOrRecovery
            }
            val recovery = lookup.recovery
    ...
    private fun beginCancelledRedundantStartStop(clientOperationId: String) {
        QuickTunnelController.updateState(
            applicationContext,
            SessionState.STOPPING,
            desiredActive = false,
        ...
        beginFailClosedRedundantStop(
            clientOperationId,
            redundantOwnerForOperation(clientOperationId),
        )
    }
    ...
    private fun handleClientStop(intent: Intent) {
        ...
        val inMemoryOperationId = inMemoryRedundantStopOperationId(
            pendingStopOperationId = pendingRedundantStop?.startOperationId,
            pendingStartOperationId = pendingOperationId,
            ownerStartOperationId = installedOwner?.startOperationId,
        )
        if (inMemoryOperationId != null) {
            beginClientRedundantStop(inMemoryOperationId, installedOwner, receiver)
            return
        }
        resolveRedundantStopFromMemoryOrRecovery(
            resolveInMemoryOperationId = {
                inMemoryRedundantStopOperationId(
                    pendingStopOperationId = pendingRedundantStop?.startOperationId,
                    pendingStartOperationId = redundantStartOperation.cancelPending(),
                    ownerStartOperationId = redundantVpnOwnerSlot.snapshot()?.startOperationId,
                )
            },
        ) { lookup ->
            val currentOwner = redundantVpnOwnerSlot.snapshot()
            if (lookup.operationId != null) {
    ...
    private fun beginClientRedundantStop(
        redundantOperationId: String,
        installedOwner: InstalledRedundantVpnOwner?,
    ...
    private fun beginLegacyClientStop(
        apiVersion: Int,
        receiver: ResultReceiver?,
    ) {
    ...
    private fun readRecoveryOffMain(
        complete: (RecoveryStoreResult<AndroidRecoveryEnvelope>) -> Unit,
    ) {
        dispatchRedundantWork(
            dispatcher = redundantWork,
            fallbackDispatcher = VPN_PROCESS_CLEANUP_WORK,
            action = {
                val recovery = recoveryStore.read()
                restoreHandler.post { complete(recovery) }
            },
            onRejected = {
                complete(RecoveryStoreResult.Failure("redundant_recovery_dispatch_rejected"))
            },
        )
    }
    ...
    private fun handleClientStatus(intent: Intent) {
        ...
        if (redundant != null) {
            ...
            ) SessionState.STOPPING else if (
                redundantOwnerForOperation(redundant.startOperationId)?.isRunning() == true
            ) {
                SessionState.RUNNING
            } else {
    ...
    private fun handleClientMetrics(intent: Intent) {
        ...
        val redundant = (recovery as? RecoveryStoreResult.Success)
            ?.value?.redundantTransaction
        if (redundant != null) {
            val active = activeRedundantTransactionForCurrentWork(
                recovery = recovery,
    ...
    private fun handleClientRebindUdp(intent: Intent) {
        ...
        if (!shouldEnterLegacyVpnRecovery(recovery)) {
            ...
            val transaction = (recovery as? RecoveryStoreResult.Success)
                ?.value?.redundantTransaction
            if (transaction != null && activeRedundantTransactionForCurrentWork(
                    recovery = recovery,
                    expectedStartOperationId = transaction.startOperationId,
                ) == null
            ) {
            ...
            redundantWork.execute {
                val current = transaction?.let {
                    activeRedundantTransactionForCurrentWork(
                        recovery = recoveryStore.read(),
    ...
    private fun handleReleaseRedundantStandby(intent: Intent) {
        ...
        if (activeRedundantTransactionForCurrentWork(
                recovery = recovery,
                expectedStartOperationId = transaction.startOperationId,
            ) == null
        ) {
        ...
        redundantWork.execute {
            val currentTransaction = activeRedundantTransactionForCurrentWork(
                recovery = recoveryStore.read(),
                expectedStartOperationId = transaction.startOperationId,
            )
    ...
    override fun onRevoke() {
        ...
        val pendingStartOperationId = redundantStartOperation.cancelPending()
        val installedOwner = redundantVpnOwnerSlot.snapshot()
        val inMemoryOperationId = inMemoryRedundantStopOperationId(
            pendingStopOperationId = pendingRedundantStop?.startOperationId,
            pendingStartOperationId = pendingStartOperationId,
            ownerStartOperationId = installedOwner?.startOperationId,
        )
        if (!redundantRevokeLifecycle.begin()) return
        if (inMemoryOperationId != null) {
            beginRedundantRevoke(inMemoryOperationId, installedOwner)
            return
        }
        resolveRedundantStopFromMemoryOrRecovery(
            resolveInMemoryOperationId = {
            ...
            val currentOwner = redundantVpnOwnerSlot.snapshot()
            if (lookup.operationId != null) {
                beginRedundantRevoke(lookup.operationId, currentOwner)
            } else {
                routeLegacyOrUnreadableVpnRevoke(
                    lookup.recovery
                        ?: RecoveryStoreResult.Failure("redundant_recovery_read_failed"),
                )
            }
        }
    }

    private fun beginRedundantRevoke(
        redundantStopOperationId: String,
        installedOwner: InstalledRedundantVpnOwner?,
    ) {
        beginFailClosedRedundantStop(
            redundantStopOperationId,
            installedOwner?.takeIf {
                it.startOperationId == redundantStopOperationId
            }?.owner,
        )
    ...
    private fun routeLegacyOrUnreadableVpnRevoke(
        recovery: RecoveryStoreResult<AndroidRecoveryEnvelope>,
    ) {
        redundantRevokeLifecycle.cleanupFinished()
    ...
    internal fun installRedundantVpnOwner(
        owner: RedundantVpnProcessOwner?,
        startOperationId: String? = null,
    ) {
        restoreHandler.removeCallbacks(redundantHealthTick)
        val previous = redundantVpnOwnerSlot.install(owner, startOperationId)?.owner
        if (previous !== owner) previous?.let(::closeOrRetainRedundantOwner)
        if (owner == null) {
            redundantPhysicalNetworks?.stop()
            redundantPhysicalNetworks = null
        }
    ...
    override fun onDestroy() {
        ...
        val pendingStartOperationId = redundantStartOperation.cancelPending()
        val installedOwner = redundantVpnOwnerSlot.snapshot()
        val redundantStopOperationId = pendingRedundantStop?.startOperationId
            ?: pendingStartOperationId
            ?: installedOwner?.startOperationId
        redundantStopOperationId?.let { operationId ->
            beginFailClosedRedundantStop(
                operationId,
                installedOwner?.takeIf { it.startOperationId == operationId }?.owner,
            )
        }
        idleStopDebouncer.cancel()
        cancelRestoreRetry()
        ...
        if (!revokePending && pendingRedundantStop == null &&
            !redundantStopLookupBarrier.hasPending()
        ) {
            installRedundantVpnOwner(null)
        }
        ...
        when {
            pendingRedundantStop != null -> Unit
            revokePending -> redundantExecutor.shutdown()
            redundantStopLookupBarrier.hasPending() -> redundantExecutor.shutdown()
            retainedRedundantOwnerCleanup.hasPending() -> redundantExecutor.shutdown()
            else -> redundantExecutor.shutdownNow()
        }
        runCatching { AutomaticDiagnostics.onTunnelStopped(applicationContext) }
            .onFailure { TunnelLog.warning("diagnostics.lifecycle_failed", error = it) }
    ...
    private fun scheduleIdleProcessRecycle(memory: AutomaticDiagnosticsProcessMemory?) {
        if (redundantStartBlocked()) return
            ...
            resolveInMemory = {
                inMemoryRedundantStopOperationId(
                    ...
                    RecoveryStoreResult.Success(
                        AndroidQuickToggleDispatch.RedundantStop(operationId),
                    )
                }
            },
            resolveDurable = {
    ...
    private fun applyBackgroundToggleResult(
        result: RecoveryStoreResult<AndroidQuickToggleDispatch>,
        receiver: ResultReceiver?,
    ) {
        val quickDispatch = when (result) {
            is RecoveryStoreResult.Success -> result.value
            is RecoveryStoreResult.Failure -> {
                redundantVpnOwnerSlot.snapshot()?.startOperationId?.let { operationId ->
                    performRedundantQuickStop(operationId, receiver)
                    return
                }
                receiver.sendError(result.code)
    ...
    private fun performRedundantQuickStop(
        startOperationId: String,
        receiver: ResultReceiver?,
    ) {
        invalidateConnectionIntentStarts()
        ...
        readRecoveryOffMain { recovery ->
            when (recovery) {
                is RecoveryStoreResult.Success -> receiver?.send(
                    SERVICE_RESULT_OK,
                    redundantStoppingConnectionIntentStatus(recovery.value).toBundle(),
                )
                is RecoveryStoreResult.Failure -> receiver.sendError("redundant_stop_pending")
            }
        }
    }
    ...
    private fun handleEnsureRunning(
        ...
        routeAndroidEnsureRunning(
            ...
            hasPendingWork = {
                redundantStartBlocked() || connectionIntentLifecycle.hasPendingWork()
            },
            desiredActive = {
                (recovery as? RecoveryStoreResult.Success)?.value?.let {
                    it.redundantTransaction?.desiredActive ?: it.intent.desiredActive
                }
            },
    ...
    private fun backgroundLogoutState(): BackgroundLogoutReadState = when (
        val result = AndroidBackgroundCredentialStores.open(applicationContext).read()
    ) {
        is CredentialStoreResult.Success -> if (
        ...
        is CredentialStoreResult.Failure -> BackgroundLogoutReadState.UNREADABLE
    }
    ...
    companion object {
        ...
        private val VPN_PROCESS_CLEANUP_EXECUTOR = Executors.newSingleThreadExecutor { task ->
            Thread(task, "nelomai-vpn-process-cleanup").apply { isDaemon = true }
        }
        private val VPN_PROCESS_CLEANUP_WORK =
            RedundantVpnWorkDispatcher(VPN_PROCESS_CLEANUP_EXECUTOR)
...
internal sealed class AndroidQuickToggleDispatch {
    ...
    data class RedundantStop(val startOperationId: String) : AndroidQuickToggleDispatch()
}
...
internal class AndroidConnectionIntentDispatchState {
    ...
    fun toggle(
        ...
        toggleLocked(expectedGeneration, durableDesiredActive, recoveryOwned, null)
    }
        ...
        when (val result = snapshot()) {
            is RecoveryStoreResult.Success -> RecoveryStoreResult.Success(
                toggleLocked(
                    ...
                    redundantStartOperationId = result.value.redundantTransaction
                        ?.startOperationId,
                ),
            )
    ...
    private fun toggleLocked(
        ...
        redundantStartOperationId: String?,
    ): AndroidQuickToggleDispatch {
        if (pendingStart || durableDesiredActive || redundantStartOperationId != null) {
            epoch = epoch.checkedIncrementDispatchEpoch()
            pendingStart = false
            return redundantStartOperationId?.let(AndroidQuickToggleDispatch::RedundantStop)
                ?: AndroidQuickToggleDispatch.Stop
        } else {
            epoch = epoch.checkedIncrementDispatchEpoch()
            pendingStart = true
...
internal class AndroidLogoutCoordinator(
    ...
    fun runOnce(
        ...
        if (connectionEnvelope.redundantTransaction != null) {
            return AndroidLogoutStep.RETRY
        }
        ...
        when (val result = connectionCoordinator.status()) {
            is RecoveryStoreResult.Failure -> return AndroidLogoutStep.RETRY
            is RecoveryStoreResult.Success -> if (result.value.redundantTransaction != null) {
                return AndroidLogoutStep.RETRY
            }
        }
...
internal fun staleConnectionIntentAction(
    ...
    hasDurableVpnServiceWork(envelope) -> AndroidStaleConnectionIntentAction.RESUME
    step == AndroidCoordinatorStep.IDLE && !envelope.intent.desiredActive ->
        AndroidStaleConnectionIntentAction.PUBLISH_STOPPED
    else -> AndroidStaleConnectionIntentAction.IGNORE
}

internal enum class BackgroundLogoutReadState {
    NONE,
    PENDING,
    UNREADABLE,
}

internal fun backgroundLogoutClientStartFailure(state: BackgroundLogoutReadState): String? =
    when (state) {
        BackgroundLogoutReadState.NONE -> null
        BackgroundLogoutReadState.PENDING -> "background_credential_logout_pending"
        BackgroundLogoutReadState.UNREADABLE -> "background_credential_unavailable"
    }
...
internal class ConnectionIntentServiceLifecycle(
    ...
    private val logoutState: () -> BackgroundLogoutReadState = {
        BackgroundLogoutReadState.NONE
    },
    ...
    private fun hasDurableWork(): Boolean =
        (coordinator.status() as? RecoveryStoreResult.Success)
            ?.value?.let(::hasDurableVpnServiceWork) == true

    private fun hasDurableConnectionWork(
        recovery: RecoveryStoreResult<AndroidRecoveryEnvelope>,
    ): Boolean = (recovery as? RecoveryStoreResult.Success)
...
internal fun redundantStoppingConnectionIntentStatus(
    envelope: AndroidRecoveryEnvelope,
): ConnectionIntentServiceStatus = ConnectionIntentServiceStatus(
    generation = envelope.intent.generation,
    desiredActive = false,
    status = "stopping",
    leasePhase = null,
