...
class LatestRuntimeStorageTest {
    ...
    private class Storage : NativeRuntimeStorageAccess {
        ...
        val proofs = (names + listOf("nelomai-quick-tunnel-state", "nelomai-redundant-cancel"))
            .associateWith { SealedRecord() }
        var quickSource = true
        var quickTarget = false
        var redundantCancelSource = false
        var redundantCancelTarget = false
        ...
        override fun hasLegacy(name: String) = when (name) {
            "nelomai-quick-tunnel-state" -> quickSource
            "nelomai-redundant-cancel" -> redundantCancelSource
            else -> sources.getValue(name).ciphertext != null
        }
        override fun retireLegacy(name: String) {
            ...
            when (name) {
                "nelomai-quick-tunnel-state" -> quickSource = false
                "nelomai-redundant-cancel" -> redundantCancelSource = false
                else -> sources.getValue(name).ciphertext = null
            }
        }
        override fun migrateQuickState() {
            if (proofs.getValue("nelomai-quick-tunnel-state").read() == null) {
                quickTarget = quickSource
                proofs.getValue("nelomai-quick-tunnel-state").write("{\"format\":1}".toByteArray())
            }
            if (proofs.getValue("nelomai-redundant-cancel").read() == null) {
                redundantCancelTarget = redundantCancelSource
                proofs.getValue("nelomai-redundant-cancel").write("{\"format\":1}".toByteArray())
            }
        }
        override fun validateQuickStateDestination(name: String) {
            val target = when (name) {
                "nelomai-quick-tunnel-state" -> quickTarget
                "nelomai-redundant-cancel" -> redundantCancelTarget
                else -> error("unexpected_plaintext_destination")
            }
            check(target) { "native_migration_destination_missing" }
        }
    }
    ...
    @Test fun redundantCancelTombstoneSurvivesPrepareRetryAndRetiresOnlyAfterAck() {
        val runtime = LatestRuntimeStorageV1()
        val storage = Storage().apply { redundantCancelSource = true }
        runtime.prepare(storage, "latest", AndroidRuntimeNamespace.version,
            legacyMigration = true, migrationComplete = false)
        assertTrue(storage.redundantCancelSource)
        assertTrue(storage.redundantCancelTarget)

        storage.redundantCancelTarget = false
        runtime.prepare(storage, "latest", AndroidRuntimeNamespace.version,
            legacyMigration = true, migrationComplete = false)
        assertFalse("receipt must fence replay over an advanced target", storage.redundantCancelTarget)
        storage.redundantCancelTarget = true

        reopenCompleted(runtime, storage)
        assertFalse(storage.redundantCancelSource)
        assertTrue(storage.redundantCancelTarget)
    }

    @Test fun missingOptionalRedundantCancelSourceDoesNotBlockMigration() {
        val runtime = LatestRuntimeStorageV1()
        val storage = Storage().apply { redundantCancelSource = false }
        runtime.prepare(storage, "latest", AndroidRuntimeNamespace.version,
            legacyMigration = true, migrationComplete = false)
        reopenCompleted(runtime, storage)
        assertFalse(storage.redundantCancelSource)
        assertFalse(storage.redundantCancelTarget)
    }
