...
async fn logout_cancels_login_waiting_for_actual_runtime_writer_before_any_issuance() {
    ...
    for already_logged_out in [false, true] {
        ...
        let broker = Arc::new(AuthBroker::new(api, auth.clone(), local.clone()).unwrap());
        ...
        let held_writer = lifecycle.lock().await;
        ...
        .unwrap();
        let login = port.login(RuntimeLogin {
        ...
        tokio::pin!(login);
        tokio::select! { biased;
            _ = &mut login => panic!("login must be waiting for the actual writer"),
            _ = tokio::task::yield_now() => {}
        }
        port.logout().await.unwrap();
        let logged_out = auth.load().unwrap();
        drop(held_writer);
        let result = tokio::time::timeout(std::time::Duration::from_secs(2), login)
            .await
            .unwrap();
        assert!(
            matches!(result, Err(nelomai_client_core::CoreError::StartCancelled)),
            "old pre-logout request must not become a fresh login: {result:?}"
        );
        assert_eq!(calls.load(Ordering::SeqCst), 0);
        assert_eq!(auth.load().unwrap(), logged_out);
