...
class AcceptanceContainerTest(ArtifactFixture):
        ...
        def native_tool(command, **kwargs):
            ...
            if command[1] == "build":
                ...
                if Path(command[0]).name == "test.AppImage":
                    ...
                    (destination / "usr/lib/Nelomai/runtime/engines/latest/0.2.21/nelomai-runtime").write_bytes(b"patched")
                else:
    ...
    def test_linux_repack_restores_only_signed_payload_and_preserves_dependencies(self):
        ...
        for relative in ("runtime/engines/stable/0.2.20/nelomai-runtime",
    ...
    def setUp(self):
        ...
        self.root_digest = aggregate.build(self.candidate, self.root / "root", "0.2.20", SOURCE, self.keyfile, self.public)
    ...
    def test_two_slots_bind_root_and_preserve_every_final_stable_byte(self):
        ...
        for item in verified["slots"][1]["manifest"]["files"]:
            extracted = resources / "engines/stable/0.2.20" / item["path"]
    ...
    def test_native_package_reextraction_rejects_changed_modes_bytes_and_extra_runtime_files(self):
        ...
        shutil.copytree(self.root / "staged", extracted)
        builder.verify_packaged_tree(self.root / "extracted", self.root / "staged", self.public, "linux", "x86_64")
        executable = extracted / "runtime/engines/stable/0.2.20/nelomai-runtime"
        original = executable.read_bytes()
        executable.write_bytes(b"rewritten after final signing")
        with self.assertRaisesRegex(ValueError, "packaged runtime bytes changed: engines/stable/0.2.20/nelomai-runtime"):
            builder.verify_packaged_tree(self.root / "extracted", self.root / "staged", self.public, "linux", "x86_64")
        executable.write_bytes(original)
        executable.chmod(0o644)
        with self.assertRaisesRegex(ValueError, "packaged runtime mode changed: engines/stable/0.2.20/nelomai-runtime.*0755.*0644"):
            builder.verify_packaged_tree(self.root / "extracted", self.root / "staged", self.public, "linux", "x86_64")
