...
def desktop(args, work, environment):
    ...
    verifier.load_script("build-runtime-manifest.py").package(payload, runtime_output, "0.2.16", args.source_sha,
        args.platform, args.architecture, args.draft_key)
    prefix = f"nelomai-runtime-0.2.16-{args.platform}-{args.architecture}"
    manifest = verifier.verify(*(runtime_output / (prefix + suffix) for suffix in (".zip", ".manifest.json", ".manifest.sig")),
        args.draft_public, "0.2.16", args.source_sha, args.platform, args.architecture)
...
def android(args, work, environment):
    ...
    verifier.load_script("build-runtime-manifest.py").package(work / "stable-stage/payload", runtime_output,
        "0.2.16", args.source_sha, "android", "aarch64", args.draft_key, readelf)
    prefix = "nelomai-runtime-0.2.16-android-aarch64"
    manifest = verifier.verify(*(runtime_output / (prefix + suffix) for suffix in (".zip", ".manifest.json", ".manifest.sig")),
        args.draft_public, "0.2.16", args.source_sha, "android", "aarch64", readelf=readelf)
    ...
    verifier.verify(*(latest / (prefix + suffix) for suffix in (".zip", ".manifest.json", ".manifest.sig")),
        args.draft_public, "0.2.16", args.source_sha, "android", "aarch64", inspect_native=False)
...
def source_archive(output, work, source):
    ...
    destination = output / "nelomai-0.2.16-amneziawg-android-source.tar.gz"
