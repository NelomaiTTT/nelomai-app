...
def desktop(args, work, environment):
    ...
    verifier.load_script("build-runtime-manifest.py").package(payload, runtime_output, gates.VERSION, args.source_sha,
        args.platform, args.architecture, args.draft_key)
    prefix = f"nelomai-runtime-{gates.VERSION}-{args.platform}-{args.architecture}"
    manifest = verifier.verify(*(runtime_output / (prefix + suffix) for suffix in (".zip", ".manifest.json", ".manifest.sig")),
        args.draft_public, gates.VERSION, args.source_sha, args.platform, args.architecture)
...
def android(args, work, environment):
    ...
    verifier.load_script("build-runtime-manifest.py").package(work / "stable-stage/payload", runtime_output,
        gates.VERSION, args.source_sha, "android", "aarch64", args.draft_key, readelf)
    prefix = f"nelomai-runtime-{gates.VERSION}-android-aarch64"
    manifest = verifier.verify(*(runtime_output / (prefix + suffix) for suffix in (".zip", ".manifest.json", ".manifest.sig")),
        args.draft_public, gates.VERSION, args.source_sha, "android", "aarch64", readelf=readelf)
    ...
    verifier.verify(*(latest / (prefix + suffix) for suffix in (".zip", ".manifest.json", ".manifest.sig")),
        args.draft_public, gates.VERSION, args.source_sha, "android", "aarch64", inspect_native=False)
...
def source_archive(output, work, source):
    ...
    destination = output / f"nelomai-{gates.VERSION}-amneziawg-android-source.tar.gz"
...
def main():
    ...
    gates.verify_source(ROOT, args.source_sha, gates.VERSION, mode=args.mode)
