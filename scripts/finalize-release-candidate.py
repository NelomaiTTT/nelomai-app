...
def main():
    ...
    gates.verify_runtime_release(args.signed / "release", gates.VERSION, args.source_sha, args.release_set_sha256, args.public_key)
    ...
    for platform, architecture in verifier.TARGETS:
        ...
        if platform == "android":
            ...
            builder.script("collect-android-release-artifact.py", "--apk", destination, "--output-dir", final_inputs,
                "--version", gates.VERSION, "--signer-sha256", matches[0])
        else:
            ...
            builder.script("collect-release-artifact.py", "--search-root", args.work, "--output-dir", final_inputs,
                "--version", gates.VERSION, "--platform", platform, "--architecture", architecture,
