...
PACKAGE_NAMES = {
    "linux": "nelomai-0.2.20-linux-x86_64.AppImage", "windows": "nelomai-0.2.20-windows-x86_64.exe",
    "macos": "nelomai-0.2.20-macos-aarch64.app.tar.gz", "android": "nelomai-0.2.20-android-aarch64.apk"}
PUBLISH_ASSETS = frozenset(PACKAGE_NAMES.values()) | {
    "nelomai-release-manifest.json", "nelomai-release-manifest.sig",
    "nelomai-0.2.20-amneziawg-android-source.tar.gz", "nelomai-0.2.20-amneziawg-android-source.tar.gz.sha256",
    "nelomai-runtime-0.2.20-release-set.manifest.json", "nelomai-runtime-0.2.20-release-set.manifest.sig",
    *(f"nelomai-runtime-0.2.20-{platform}-{architecture}{suffix}"
...
def verify_source(root, source, version, *, base=BASE, mode="sign_candidate"):
    ...
    full_source(source)
    if version != "0.2.20":
        raise ValueError("this maintenance workflow only builds 0.2.20")
...
def verify_retained_artifact(metadata, archive, directory, artifact_id, run_id, source, repository_id):
    ...
    if (str(metadata.get("id")) != str(artifact_id) or not re.fullmatch(r"[1-9][0-9]*", str(artifact_id))
            or metadata.get("name") != "candidate-0.2.20" or metadata.get("expired") is not False
...
def verify_updater_release(directory, runtime_public_key, updater_public_key, android_signer_sha256):
    ...
    if (document.get("schema_version") != 1 or document.get("version") != "0.2.20" or len(artifacts) != 4
...
def main():
    ...
    if args.command == "source":
        ...
        verify_runtime_release(args.directory, "0.2.20", args.source_sha, args.release_set_sha256, args.public_key)
