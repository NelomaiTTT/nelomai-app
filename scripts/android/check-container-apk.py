...
def verify_manifest(xml, acceptance=False):
    ...
    if acceptance:
        ...
        raise ValueError('0.2.19 shipping manifest must be latest-only')
...
def verify_classes(classes, acceptance=False):
    ...
    if acceptance:
        ...
        raise ValueError('0.2.19 container must be latest-only')
...
def verify_slots(manifest, acceptance=False, root_digest=None, stable_digest=None):
    ...
    if not acceptance:
        if [slot['slot'] for slot in slots] != ['latest']:
            raise ValueError('0.2.19 container must index only latest')
        return
...
def main():
    ...
    parser.add_argument('--acceptance', action='store_true', help='Separate two-slot test package only; never shipping 0.2.19')
