...
shipping 0.2.20 continues to use the existing latest-only container stage.
"""
import argparse
...
def stage_android_latest(payload, output, source, readelf):
    ...
    if ({name: manifest.get(name) for name in ("format_version", "runtime_version", "source_commit",
            "platform", "architecture", "contract_version")} != dict(format_version=1, runtime_version="0.2.20",
...
def stage(candidate, root_digest, latest_payload, output, signing_key, public_key,
    ...
    gates = verifier.load_script("release-candidate-gates.py")
    gates.verify_runtime_release(candidate, "0.2.20", source, root_digest, public_key)
    ...
    with tempfile.TemporaryDirectory(prefix=".acceptance-stage-", dir=output.parent) as temporary:
        ...
        stable = verifier.verify(*stable_inputs, public_key, "0.2.20", source, platform, architecture,
                                 readelf=readelf, output=runtime / "engines/stable/0.2.20")
        if platform == "android":
        ...
        container = dict(format_version=1, container_version="0.2.20",
