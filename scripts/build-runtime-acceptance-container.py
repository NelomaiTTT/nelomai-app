...
shipping 0.2.17 continues to use the existing latest-only container stage.
"""
import argparse
...
def stage_signed(signed, root_digest, output, public_key, platform, architecture, source, kind, readelf=None):
    ...
    verifier.load_script("release-candidate-gates.py").verify_runtime_release(
        signed / "release", "0.2.17", source, root_digest, public_key)
    ...
    with tempfile.TemporaryDirectory(prefix=".keyless-stage-", dir=output.parent) as temporary:
        ...
        for slot in container["slots"]:
            ...
            original = verifier.verify(archive, folder / (prefix + ".manifest.json"),
                folder / (prefix + ".manifest.sig"), public_key, "0.2.17", source, platform, architecture,
...
def stage_android_latest(payload, output, source, readelf):
    ...
    if ({name: manifest.get(name) for name in ("format_version", "runtime_version", "source_commit",
            "platform", "architecture", "contract_version")} != dict(format_version=1, runtime_version="0.2.17",
...
def stage(candidate, root_digest, latest_payload, output, signing_key, public_key,
    ...
    gates.verify_runtime_release(candidate, "0.2.17", source, root_digest, public_key)
    ...
    with tempfile.TemporaryDirectory(prefix=".acceptance-stage-", dir=output.parent) as temporary:
        ...
        stable = verifier.verify(*stable_inputs, public_key, "0.2.17", source, platform, architecture,
                                 readelf=readelf, output=runtime / "engines/stable/0.2.17")
        if platform == "android":
...
def package_desktop(staged, output, public_key, platform, architecture, *, root=ROOT, environment=None,
    ...
    name = f"nelomai-acceptance-0.2.17-{platform}-{architecture}"
    ...
    if platform == "macos":
        ...
        subprocess.run([str(plugin), "--appimage-extract-and-run", "--appdir", str(intermediate / "squashfs-root")],
            env={**environment, "APPIMAGE_EXTRACT_AND_RUN": "1", "ARCH": architecture,
                 "OUTPUT": str(package), "VERSION": "0.2.17"}, cwd=output, check=True)
...
def package_android(staged, output, public_key, readelf, apkanalyzer, common_host, *, root=ROOT,
    ...
    stable_aar = runtime / "engines/stable/0.2.17/runtime/runtime.aar"
    ...
    package = output / "nelomai-acceptance-0.2.17-android-aarch64.apk"
